...
var_imu_f = 0.01
var_imu_w = 0.01
var_gnss  = 0.001  # Strongly trust GNSS position measurements (10x more than before)
var_lidar = 0.1    # Strongly trust LIDAR position measurements (5x more than before)
# Exponential moving average alpha for velocity smoothing (0 < alpha <= 1). Lower => more smoothing
vel_ema_alpha = 0.3
...
# Set initial values.
p_est[0] = gt.p[0]
# Initialize velocity to zero to let the filter estimate it from measurements
# Using ground truth velocity in simulation can bias the filter
v_est[0] = np.zeros(3)  # Changed from gt.v[0] to zeros
q_est[0] = Quaternion(euler=gt.r[0]).to_numpy()
# Normalize initial quaternion to ensure unit length
q_est[0] = q_est[0] / np.linalg.norm(q_est[0])
# Increase initial covariance slightly to allow faster convergence
p_cov[0] = np.eye(9) * 0.5
gnss_i  = 0
lidar_i = 0
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    # Normalize quaternion to prevent numerical drift and ensure unit length
    ...
    F[3:6, 6:] = -rotation_matrix.dot(skew_symmetric(imu_f.data[k-1]))
    ...
    # Measurement synchronization: match measurements to current IMU time (imu_f.t[k])
    lidar_updated = False
    gnss_updated = False
    
    if lidar_i < lidar.t.shape[0] and abs(lidar.t[lidar_i] - imu_f.t[k]) < 1e-2:
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_lidar, p_cov[k], lidar.data[lidar_i], p_est[k], v_est[k], q_est[k])
        lidar_i += 1
        lidar_updated = True
    if gnss_i < gnss.t.shape[0] and abs(gnss.t[gnss_i] - imu_f.t[k]) < 1e-2:
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss, p_cov[k], gnss.data[gnss_i], p_est[k], v_est[k], q_est[k])
        gnss_i += 1
        gnss_updated = True

    # 4. Velocity smoothing (exponential moving average) to suppress high-frequency spikes
    try:
        v_est[k] = vel_ema_alpha * v_est[k] + (1.0 - vel_ema_alpha) * v_est[k-1]
    except Exception:
        pass
    
    # Debug output every 50 samples
    if k % 50 == 0:
        print(f"Step {k}: LIDAR={lidar_updated}, GNSS={gnss_updated}, Pos=[{p_est[k,0]:.2f}, {p_est[k,1]:.2f}, {p_est[k,2]:.2f}]")
...
print(f"Position error magnitude: {np.linalg.norm(p_est[-1] - gt.p[-1]):.4f} m")
print(f"Sensor variances - IMU_F: {var_imu_f}, IMU_W: {var_imu_w}, GNSS: {var_gnss}, LIDAR: {var_lidar}\n")
